...
library(gridExtra)# for plotting grid plots
...
# check for NA or NaN values in the dataframe (clValid will not know how to cluster)
sum(is.nan(df))
sum(is.na(df))
...
# run clustering across a range of K and algorithms to identify optimal clustering approach for this measure

# internal validity: Connectvity (lower better), Dunns Index (higher better), Sillhoette (higher better)
intern<- clValid(df, 2:6, clMethods=c("kmeans", "pam", "hierarchical"), validation = "internal")

# summarise the results of clValid
summary(intern)
...
optimal_k <- 3
optimal_algorithm <- "hierarchical"

if (optimal_algorithm=="hierarchical"){
  #### Plot and evaluate hierarchical clustering ####
  print("selected algorithm was hierarchical")
  
  # extract the hierarchical cluster object from intern
...
if (optimal_algorithm=="k-means"){
  #### Plot and evaluate k-means clustering ####
  k_df<-df
  k_df$cluster <- as.factor(intern@clusterObjs$kmeans[[optimal_k]]$cluster)
  fviz_cluster(intern@clusterObjs$kmeans[[optimal_k]], df, ellipse.type = "euclid")
